...
**Content system:** Project case studies live in `src/content/projects/*.mdx`. The collection schema is defined in `src/content.config.ts` (fields: `title`, `description`, `techStack`, optional `order`, `image`, `publishedDate`, `updatedDate`). Pages are generated via `src/pages/projects/[slug].astro` using `getStaticPaths()` with the slug derived from the MDX filename. `publishedDate`/`updatedDate` (when present) flow into the per-page `TechArticle` JSON-LD.

**MDX case study conventions:** Case study bodies do NOT include code snippets — the prose describes engineering decisions. The opening `> blockquote` on each page renders as a duotone-accented lead quote (see `.prose-content blockquote` in `global.css`), so frame it as a one-liner tagline for the project.
